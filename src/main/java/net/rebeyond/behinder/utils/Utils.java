...
public class Utils {
    ...
    //重写协商密钥
    public static Map<String, String> regetKeyAndCookie(String getUrl, String password, Map<String,String> requestHeaders) throws Exception {
        disableSslVerification();//禁止SSL加密
        Map<String, String> result = new HashMap<>();
        StringBuffer sb = new StringBuffer();
        InputStreamReader isr = null;
        BufferedReader br =null;

        URL url = new URL(getUrl);//无拼接
        ...
        //1.检测http或https，2.检测是否有代理
        if(url.getProtocol().equals("https")){
            if (Main.currentProxy != null){
                    urlConnection = url.openConnection(Main.currentProxy);//开连接
                    if(Main.proxyUserName!=null && !Main.proxyUserName.equals("")){
                        ...
                        ((HttpURLConnection) urlConnection).setRequestProperty(urlwithSession,headerValue);
                    }
                    else {
                        urlConnection = url.openConnection();//开连接
                    }
        ...
        if(status == 301 || status ==302){
            //重定向的话则将目标locaiton url赋值给urlwithSession
            ...
            if (error){
                ...
                String oncekey = sb.toString();
                System.out.println("oncekey:"+oncekey+"\n\n");

                String pattern = "[a-fA-F0-9]{16}";
                Pattern r = Pattern.compile(pattern);
                Matcher m = r.matcher(oncekey);
                if(!m.find()){
    ...
    public static Map<String, String> getKeyAndCookie(String getUrl, String password, Map<String, String> requestHeaders) throws Exception {
        ...
        InputStreamReader isr = null; //输入流
        BufferedReader br = null; //缓冲读取流
        ...
        HttpURLConnection.setFollowRedirects(false);//所有http连接自动处理重定向
        Object urlConnection;
        String urlwithSession;
        String errorMsg;
        //设置代理
        if (url.getProtocol().equals("https")) {
            if (Main.currentProxy != null) {
                ...
                urlConnection = url.openConnection();//https连接
            }
        } else if (Main.currentProxy != null) {
            urlConnection = url.openConnection(Main.currentProxy);
            if (Main.proxyUserName != null && !Main.proxyUserName.equals("")) {
            ...
            urlConnection = url.openConnection();//http连接
        }

        for (String s : requestHeaders.keySet()) {
            urlwithSession = s; //循环取请求头的值对
            ((HttpURLConnection) urlConnection).setRequestProperty(urlwithSession, requestHeaders.get(urlwithSession));
        }
        urlwithSession = "";
        ...
        System.out.println("getrawkey.headers:"+headers.toString());
        String line1;
        while (var12.hasNext()) {
            String headerName = var12.next();
            if (headerName != null && headerName.equalsIgnoreCase("Set-Cookie")) {
                for (Iterator var14 = ((List) headers.get(headerName)).iterator(); var14.hasNext(); urlwithSession = urlwithSession + ";" + line1) {
                    line1 = (String) var14.next();
                }
                System.out.println("oldcookie:"+urlwithSession);
                urlwithSession = urlwithSession.startsWith(";") ? urlwithSession.replaceFirst(";", "") : urlwithSession;
                System.out.println("newcookie:"+urlwithSession);
                result.put("cookie",urlwithSession);
                break;
            }
        }
        ...
        if (((HttpURLConnection) urlConnection).getResponseCode() == 500) {
            ...
            isr = new InputStreamReader(((HttpURLConnection) urlConnection).getInputStream());// 获取输入流
        }
        ...
        if (error) {
            ...
            String pattern = "[a-fA-F0-9]{16}";//16进制正则
            Pattern r = Pattern.compile(pattern);
            ...
            if (!m.find()) {
                ...
                String finalKey = rawKey_1;
                result.put("key", finalKey);

                //System.out.println("getkeyandcookie:"+result.toString());
                return result;
                ...
                while (true) {
                    ...
                    System.out.println("rawKey_2:"+rawKey_2+"\n\n");

                    //
                    byte[] temp = CipherUtils.bytesXor(rawKey_1.getBytes(), rawKey_2.getBytes());//异或操作
                    ...
                    //if (end - start == 16) {
                        ...
                        result.put("endIndex", String.valueOf(temp.length - end));
                        //String finalKey = new String(Arrays.copyOfRange(rawKey_2.getBytes(), start, end));
                        String finalKey = "8c25dfbedf9e402a";
                        result.put("key", finalKey);
    ...
    public static Map<String, String> getRawKey(String getUrl, String password, Map<String, String> requestHeaders) throws Exception {
        ...
        //System.out.println(requestHeaders.keySet().toString());
        for (String s : requestHeaders.keySet()) {
            cookieValues = s;
        ...
        while (var12.hasNext()) {
            ...
            if (headerName != null && headerName.equalsIgnoreCase("Set-Cookie")) {
                ...
                //System.out.println("oldcookie:"+cookieValues);
                cookieValues = cookieValues.startsWith(";") ? cookieValues.replaceFirst(";", "") : cookieValues;
                //System.out.println("newcookie:"+cookieValues);
                break;
            }
        }
        ...
        if (error) {
            ...
            //System.out.println("getrawkey:"+result.toString());
            return result;
        }
    }
    ...
    public static byte[] getData(String key, int encryptType, String className, Map<String, String> params, String type, byte[] extraData) throws Exception {
        ...
        if (type.equals("jsp")) {
            ...
            encrypedBincls = Crypt.Encrypt(bincls, key);//key
            String basedEncryBincls = Base64.getEncoder().encodeToString(encrypedBincls);
